...
const Sphere spheres[] = {
	...
	{ 1e5,   { 0.0, 40.8, 1e5 },		   { 0.0, 0.0, 0.0 },    { 0.75, 0.75, 0.75 },    DIFFUSE},	   //Back
...
int main(int argc, char* argv[]) {
	...
	const Vector3 position = { 50, 52, 295.6 };
	Vector3 direction = { 0, -0.042612, -1 };
	normalize_v3(&direction);
	const double fov = 0.5135;
	const Vector3 cx = { w * fov / h, 0.0, 0.0 };
	Vector3 _cy = cross_v3v3(&cx, &direction);
	normalize_v3(&_cy);
			...
			for (unsigned int sy = 0u, i = (h - 1u - y) * w + x; sy < 2u; ++sy) { // 2 subpixel row
				...
				for (unsigned int sx = 0u; sx < 2u; ++sx) { // 2 subpixel column
					...
					for (unsigned int s = 0u; s < nb_samples; ++s) { // samples per subpixel
						...
						Vector3 d = add_v3v3(&_ab, &direction);
						const Vector3 d130 = mul_v3d(&d, 130.0);
						Ray ray = { add_v3v3(&position, &d130), *normalize_v3(&d), EPSILON_SPHERE, INFINITY, 0 };
